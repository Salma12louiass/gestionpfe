// app/livrables/LivrablesContent.jsx
"use client";
...
const LivrablesContent = () => {
  ...
  const handleValidation = async (idLivrable, statut) => {
    ...
    if (user.role !== 'responsableFiliere') {
  ...
  const canEdit = (livrable) => {
    ...
    // Les responsables peuvent modifier tous les livrables non validés
    return user.role === 'responsableFiliere' && livrable.statut !== StatutLivrable.Validé;
  };
  ...
  const canValidate = () => {
    // Seuls les responsables peuvent valider/rejeter des livrables dans cette vue
    return user.role === 'responsableFiliere';
  };
          ...
          <nav className="bg-white shadow-md p-4 rounded-lg mb-6 dark:bg-gray-800">
            <div className="flex items-center justify-between">
              <h1 className="text-xl font-bold text-gray-800 flex-grow text-center mx-4 dark:text-white">
                {user?.role === 'etudiant' ? '📁 Mes Livrables' : '📁 Gestion des Livrables'}
              </h1>
...
export default LivrablesContent;